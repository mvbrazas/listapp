...
namespace ListApp.Services
{
	public class NavigationService
	{
        ...
        {
            return CurrentNavigationPage.Navigation.NavigationStack[0];
        }

        public CustomMainPage MainPage()
        {
            return GetRootPage() as CustomMainPage;
        }

        public List<string> GetStackPages()
        {
            var mainPage = MainPage();
            var stackPageString = new List<string>();
            var stackPage = mainPage.Navigation.NavigationStack;
            for (int x = 0; x < stackPage.Count; x++)
            {
                stackPageString.Add(stackPage[x].ToString());
            }

            return stackPageString;
        }
        ...
        {
            Configure("MainPage", typeof(MainPage));
            Configure("UserPage", typeof(UserPage));
        }

        public async Task OpenUserPage(Person user)
        {
            await NavigateAsync(nameof(UserPage), user);
        }
    }
}
